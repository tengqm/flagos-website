...
FLIR (FlagTree Linalg Intermediate Representation) is a multi-backend unified intermediate layer that serves as the central hub for lowering Triton extensions intermediate representations (for example, Hints, Ops, and TLE) to hardware-specific dialect. When you use Hints and TLE features, FLIR feature is automatically used without user interventions.
...
|----------|-------------|
| **Language Coverage** | Supports 80 Triton language primitives and xx operators. |
